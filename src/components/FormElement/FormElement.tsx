...
function FormElement({
    ...
    const handleChange = (
        ...
        if (type === FormInput.Radio) {
            const query =
                `input[name='${name}']:checked` as keyof HTMLElementTagNameMap;
            const checkedInput = e.target
                ?.closest<HTMLDivElement>(`.${formElementWrapperClassName}`)
                ?.querySelector(query) as HTMLInputElement | null;
            if (!checkedInput) {
                value = '';
            }
    ...
    return (
        <div className={formElementWrapperClassName} ref={formEntryRef}>
            {type !== FormInput.Checkbox ? (
                <label className="popup__label label" htmlFor={id}>
                    {label}
                </label>
            ) : null}
            {renderInput()}
