...
type TaskPopupProps = {
    ...
    onTaskConfirm: (id: number | undefined, taskData: TaskData) => Promise<void>
...
function TaskPopup({ task, onTaskConfirm, onTaskPopupClose }: TaskPopupProps) {
    ...
    const [taskDataFields, setTaskDataFields] = useState<TaskFormData>(getTaskDataFromTask(task))
    const TaskDataKeys = Object.keys(taskDataFields).reduce((acc, key) => ({ ...acc, [key]: key }), {});
    ...
    const handleChangeClosedState = ((key: keyof TaskFormData, value: string | boolean) => {
        setTaskDataFields((prevState: TaskFormData) => {
    ...
    const handleConfirm = async() => {
        ...
        const editedTaskData: TaskData = {
            ...
            description: taskDataFields.description?.trim().length ? taskDataFields.description?.trim() : undefined,
            ...
            note: taskDataFields.note?.trim().length ? taskDataFields.note.trim() : undefined
        }
